...
def extract_raw_4ch(data):
    audio = np.frombuffer(data, dtype=np.int16)
    audio = audio.reshape(-1, RESPEAKER_CHANNELS)
    return audio[:, 1:5]


def calculate_level_dbfs(audio_int16):
    audio = audio_int16.astype(np.float64) / 32768.0

    # Remove the DC offset independently from each microphone.
    audio = audio - np.mean(audio, axis=0, keepdims=True)

    # Array-level power: average the power, not the dB values.
    mean_power = np.mean(audio**2)
    array_rms = np.sqrt(mean_power + 1e-12)
    return float(20 * np.log10(array_rms + 1e-12))


def measure_source(mic_tuning, stream, sound_index):
    input(f"Prepare sound {sound_index}, then press Enter to measure DoA and level for 10 seconds...")
    print(f"Measuring DoA and received level for sound {sound_index}...")

    angles = []
    audio_blocks = []
    failed_reads = 0
    end_time = time.monotonic() + DOA_MEASURE_SECONDS
    next_doa_sample_time = 0.0

    while time.monotonic() < end_time:
        data = stream.read(CHUNK, exception_on_overflow=False)
        audio_blocks.append(extract_raw_4ch(data))

        now = time.monotonic()
        if now < next_doa_sample_time:
            continue

        next_doa_sample_time = now + DOA_SAMPLE_INTERVAL
        try:
    ...
    if not audio_blocks:
        raise RuntimeError(f"No audio samples were recorded while measuring sound {sound_index}.")
    ...
    received_level_dbfs = calculate_level_dbfs(np.vstack(audio_blocks))

    print(f"Sound {sound_index}: DoA = {average_doa} degrees, received level = {received_level_dbfs:.2f} dBFS")

    return {
        "sound_index": sound_index,
        "doa": average_doa,
        "received_level_dbfs": received_level_dbfs,
    }
...
def build_output_paths(output_dir, source_count, doas):
    doa_labels = [str(doa) if doa is not None else "NA" for doa in doas]
    recording_id = get_next_recording_id(output_dir)
    wav_path = output_dir / f"fileid_{recording_id}_sources_{source_count}_{doa_labels[0]}_{doa_labels[1]}_{doa_labels[2]}.wav"
    level_path = output_dir / f"fileid_{recording_id}_sound_level.txt"
    return recording_id, wav_path, level_path
...
def save_sound_levels(level_path, recording_id, source_count, measurements):
    with open(level_path, "w", encoding="utf-8") as f:
        f.write(f"file_id: {recording_id}\n")
        f.write(f"sound_sources: {source_count}\n")
        f.write(f"calibration_seconds: {DOA_MEASURE_SECONDS:.1f}\n\n")

        for measurement in measurements:
            f.write(f"sound_{measurement['sound_index']}:\n")
            f.write(f"  doa_degrees: {measurement['doa']}\n")
            f.write(f"  received_level_dbfs: {measurement['received_level_dbfs']:.2f}\n\n")
...
def main():
    ...
    try:
        ...
        for sound_index in range(1, source_count + 1):
            measurement = measure_source(mic_tuning, stream, sound_index)
            measurements.append(measurement)
            doas[sound_index - 1] = measurement["doa"]

        recording_id, output_path, level_path = build_output_paths(OUTPUT_DIR, source_count, doas)
        print(f"Output file will be: {output_path}")
        print(f"Sound levels will be saved as: {level_path}")
        ...
        save_sound_levels(level_path, recording_id, source_count, measurements)
        print(f"Recording saved as {output_path}")
        print(f"Sound levels saved as {level_path}")
